...
AC_DEFUN([BFD_128_BIT], [dnl
# See whether 128-bit bfd lib has been enabled.
...
# Note we cannot cache the result of this check because BFD128 may change
# when a secondary target has been added or removed and we have no access
# to this information here.
AC_MSG_CHECKING([whether BFD is 128-bit])
AC_EGREP_CPP([HAVE_BFD128],
  AC_LANG_PROGRAM(
    [#include "bfd.h"],
    [dnl
#ifdef BFD128
HAVE_BFD128
#endif]),
  [have_128_bit_bfd=yes],
  [have_128_bit_bfd=no])
AC_MSG_RESULT([$have_128_bit_bfd])
