		   ...
		   uint64_t *nrelrsp)
{
  ...
  for (i = 0; i < nentries; i++)
	...
	BYTE_SETGET(entry, ((Elf128_External_Relr *)relrs)[i].r_data);
      if ((entry & 1) == 0)
	size++;
      else
	while ((entry >>= 1) != 0)
		  ...
		  bool is_dynsym)
{
  ...
  if (rel_type == reltype_rela)
	...
	if (!is_32bit_elf && !is_64bit_elf)
	  printf ("%016" PRIx64 "%016" PRIx64 "\n", (unsigned long) ((__uint128_t) relrs[i] >> 64), relrs[i]);
	else
...
#define CHECK_ENTSIZE(section, i, type)					\
  ...
  for (i = 0, section = filedata->section_headers;
	...
	print_symbol (is_32bit_elf || is_64bit_elf ? -17 : -33, section_name_print (filedata, section));
